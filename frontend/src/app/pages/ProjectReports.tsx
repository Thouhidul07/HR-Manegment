...
export function ProjectReports() {
  ...
  const reportRows = () => [
    ...
    ...taskCompletionData.map((item) => [item.date, item.completed, item.inProgress, item.todo]),
    [],
    ["Project Distribution"],
    ["Project", "Percent"],
    ...projectDistribution.map((item) => [item.name, item.value]),
    [],
    ["Team Performance"],
  ...
  const exportCsv = () => {
    const csv = reportRows().map((row) => row.map((cell) => csvCell(cell ?? "")).join(",")).join("\n");
  ...
  const exportPdf = () => {
    const printableRows = reportRows()
      .map((row) => {
        ...
        return `<tr>${row.map((cell) => `<td>${String(cell)}</td>`).join("")}</tr>`;
      })
      .join("");
