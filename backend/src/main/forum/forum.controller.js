...
const listPosts = asyncHandler(async (req, res) => {
  ...
  if (req.query.category && !["all", "All Topics"].includes(req.query.category)) {
    filters.push("fp.category = ?");
    params.push(req.query.category);
  }
  if (req.query.search) {
    filters.push("(fp.title LIKE ? OR fp.body LIKE ? OR fp.category LIKE ? OR fp.tags LIKE ?)");
    params.push(
      `%${req.query.search}%`,
      `%${req.query.search}%`,
      `%${req.query.search}%`,
  ...
  if (req.query.sentiment && req.query.sentiment !== "all") {
    filters.push("fp.sentiment = ?");
    params.push(req.query.sentiment);
  }
  if (req.query.type === "polls") {
    filters.push("fp.is_poll = TRUE");
  }
  if (req.query.type === "discussions") {
    filters.push("fp.is_poll = FALSE");
  }

  const sortMap = {
    popular: "likes DESC, fp.views DESC",
    discussed: "reply_count DESC",
    unanswered: "reply_count ASC, fp.created_at DESC",
  ...
  res.json({
    posts: rows.slice(0, limit).map(mapPost),
